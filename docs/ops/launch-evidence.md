...
| Baseline monitoring   | Uptime, error-rate, queue, failed-job, scheduler, DB, Redis | Blocked | Ops   | Local service checks passed for Nginx, PHP-FPM, MariaDB, and Talabix queue/scheduler systemd services. Blocked: monitoring provider, log drain, uptime/error/queue dashboard links, and alert destination are not available in this workspace |
...
| Runtime              | Native PHP 8.3 FPM, MariaDB 10.11, Node 20, Nginx                                                                                             |
...
| API readiness        | Pass: keyed `https://smartcedra.online/talabix/api/v1/readiness` returned HTTP 200 and `status=ready`                                         |
...
| Background services  | Pass: `talabix-queue.service`, `talabix-scheduler.service`, `php8.3-fpm`, `nginx`, and `mariadb` reported `active`                            |
